...
public class ConfigCenter {
    ...
    @Value("${admin.metadata-report.username:}")
    private String metadataUsername;

    @Value("${admin.metadata-report.password:}")
    ...
    Registry getRegistry() {
        ...
        if (registryUrl == null) {
            if (StringUtils.isBlank(dubboRegistryAddress)) {
                throw new ConfigurationException("Either config center or registry address is needed, please refer to https://github.com/apache/incubator-dubbo-admin/wiki/Dubbo-Admin-configuration");
            }
            registryUrl = formUrl(dubboRegistryAddress, registryGroup, registryNameSpace, username, password);
        }
    ...
    MetaDataCollector getMetadataCollector() {
        ...
        if (metadataUrl == null) {
            if (StringUtils.isNotEmpty(metadataAddress)) {
                metadataUrl = formUrl(metadataAddress, metadataGroup, metadataGroupNameSpace, metadataUsername, metadataPassword);
